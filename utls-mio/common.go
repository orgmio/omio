...
import (
	...
	"github.com/orgmio/utls-mio/internal/fips140tls"
)
...
type Config struct {
	...
	// configurations are guaranteed to use Rand. Go 1.26+ crypto APIs ignore
	// this Reader unless GODEBUG=cryptocustomrand=1. Test code can use
	// [testing/cryptotest.SetGlobalRandom] instead.
	Rand io.Reader

	// InsecureSkipCertificateVerify skips verification of the TLS 1.3
	// CertificateVerify signature. It is an ixa-go protocol extension and must
	// only be enabled when the peer is authenticated independently. It does not
	// remove the CertificateVerify message from the handshake transcript.
...
func (c *Config) Clone() *Config {
	...
	return &Config{
		...
		InsecureSkipCertificateVerify:       c.InsecureSkipCertificateVerify,
