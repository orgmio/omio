...
func TestMain(m *testing.M) {
	...
	// Go 1.26+ ignores Config.Rand inside crypto/ecdh and crypto/rsa unless
	// cryptocustomrand=1. Handshake transcripts in testdata were recorded
	// against zeroSource; keep honoring Config.Rand in the test binary.
	// Production (Config.Rand nil) still uses the process DRBG, matching Go 1.27.
	if v := os.Getenv("GODEBUG"); v == "" {
		os.Setenv("GODEBUG", "cryptocustomrand=1")
	} else {
		os.Setenv("GODEBUG", v+",cryptocustomrand=1")
	}
